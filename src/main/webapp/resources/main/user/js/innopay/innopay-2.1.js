let ediDate='';
let payActionUrl="https://pg.innopay.co.kr";
let device='';
let keyStr="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
let iw = 680;
let ih = 680;
let formName='InnopayForm';
let rtn;
let flag=false;
let m_debug=false;
let m_test=false;
...
function setEdiDate(f){
	...
	let strTime = hours.toString() + minutes.toString() + seconds.toString();
...
let scroll_disable = function(){
	  $('html, body').addClass('hidden');
	};
	let scroll_enable = function(){
		  $('html, body').removeClass('hidden');
		};
function setMoid(frm){
	if(!checkFormField(frm,'Moid')||!checkFormValue(frm,'Moid')){
		let a = jQuery('input[name=MID]',jQuery(frm)).val();
...
let innopay={
    goPay:function(data){
    			...
    			let formObj = jQuery('form[name='+formName+']');
    			formObj.submit();
    },
	closeDiv:function(){
		let obj=document.getElementById('innoDiv');
		  try{
	...
	goPayForm:function(pform){
		let arrObj=jQuery(pform).serializeArray();
    	let fmobj = {};
	...
	goAcct:function(){
	    loadAcctForm();
	    let formObj = jQuery('form[name=InnoAcct]');
        formObj.submit();
	}
};
function init(data){
	let f = createForm(formName,'POST','','');
	setEdiDate(f);
	setPayActionUrl();
	...
	let rst=checkData(f,data);
	if(!rst) return false;
	makeEncKey(f);
	let fwd=jQuery('input[name=FORWARD]',jQuery(f)).val();
	f.action=payActionUrl+'/ipay/interfaceURL.jsp';
	if('X'==fwd){
		let lPos=(window.innerWidth-iw)/2;
		let tPos=(window.innerHeight-ih)/2;
		...
		let innoDiv = document.createElement('div');
		...
		let left=(window.innerWidth-iw)/2;
		let top=(window.innerHeight-ih)/2;
		let winopts= "left="+left+",top="+top+",toolbar=no,location=no,directories=no, status=no,menubar=no,scrollbars=no, resizable=no,width="+iw+"px,height="+ih+"px";			
		let InnopayWin =  window.open("about:blank", "payWindow", winopts);
...
function makeEncKey(frm){
	let strKey = '';
	let MID=jQuery('input[name=MID]', jQuery(frm)).val();
	let Amt=jQuery('input[name=Amt]', jQuery(frm)).val();
	let mKey=jQuery('input[name=MerchantKey]', jQuery(frm)).val();
	let DutyFreeAmt = 0;
	try{
	...
	let enc_val = encode64(MD5(strKey));
...
function createForm(name, method, action, target){
	let tmpForm = jQuery('form[name='+name+']');
	let formObj;
...
function checkFormField(CheckForm, CheckFormFieldName){
    try{
	  let a=jQuery('input[name='+CheckFormFieldName+']', jQuery(CheckForm)).val();
...
function addHidden(formObj, name, value){
	if(checkFormField(formObj,name)){
		...
		let m_input = getInputTag('hidden', name, value);
		try{
			formObj.appendChild(m_input);	
		}catch(e){
			let fm = jQuery('form[name='+formName+']')[0];
...
function loadAcctForm(){
    LoadEvent();
    let acctFormName = 'InnoAcct';
    let f = createForm(acctFormName,'POST','','');
    setPayActionUrl();
    device=checkDevice(f);
    let lPos=(window.innerWidth-iw)/2;
    let tPos=(window.innerHeight-ih)/2;
    ...
    if(window.addEventListener){
        window.addEventListener('message',function(e){
            if(flag == true){
                ...
                try{
                    let str = decodeURIComponent(e.data);
